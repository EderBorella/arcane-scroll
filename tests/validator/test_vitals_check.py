...
def _core_sheet(classes=None, hit_dice=None, hp_max=22, con_final=14, species="Species A"):
    """Top-level-fields sheet shape: hit_points / hit_dice live at the document root, not nested
    under a combat object."""
    classes = classes if classes is not None else [{"class": "Class A", "level": 3}]
    total_level = sum(c["level"] for c in classes)
    con_mod = (con_final - 10) // 2
    return {
...
def test_core_clean_sheet_has_no_findings(access):
    assert check(_core_sheet(), access) == []


def test_core_hp_out_of_range(access):
    # HP-range must fire on the top-level fields -- reading only a combat sub-object no-ops here.
    assert "hp-out-of-range" in _codes(_core_sheet(hp_max=999), access)


def test_core_hit_dice_total_mismatch(access):
    # Hit-dice pool must fire on the top-level fields too.
    s = _core_sheet(hit_dice={"d8": {"max": 2, "remaining": 2}})
    assert "hit-dice-total-mismatch" in _codes(s, access)


def test_core_hp_out_of_range_path_uses_top_level(access):
    findings = [v for v in check(_core_sheet(hp_max=999), access) if v.code == "hp-out-of-range"]
    assert findings and findings[0].path == "hit_points.max"
