...
def _clean_stat_block(index=0):
    """A hand-built companion-modifier stat block matching creature-c exactly, built
    from the rule/catalog facts (not the deriver output)."""
    return {
...
def _monster(creature_id="creature-c", stat_block=None, index=0):
    return {"creature_id": creature_id,
            "stat_block": stat_block if stat_block is not None else _clean_stat_block(index)}
...
class TestCleanSheetPasses:
    ...
    def test_multiple_monsters_clean(self, access):
        sheet = _sheet([_monster(index=0), _monster(index=1)])
        assert check(sheet, access) == []


# ── RED: a wrong stat is flagged ─────────────────────────────────────────────
...
class TestWrongStatFires:
    ...
    def test_wrong_speed_fires(self, access):
        ...
        sb["speed"] = {"walk": 99}
        codes = {v.code for v in check(_sheet([_monster(stat_block=sb)]), access)}
        assert "companion-speed-mismatch" in codes
    ...
    def test_wrong_stat_finding_is_retagged_to_monster_domain_and_path(self, access):
        ...
        v = next(x for x in check(_sheet([_monster(stat_block=sb)]), access)
                 if x.code == "companion-hp-mismatch")
    ...
    def test_retag_fires_for_non_hp_field_armor_class(self, access):
        # Guard against silent path/message format drift in the reused companion
        # helpers for a field OTHER than HP (armor_class lives at a different depth).
        sb = _clean_stat_block()
        sb["armor_class"] = 99
        v = next(x for x in check(_sheet([_monster(stat_block=sb)]), access)
                 if x.code == "companion-ac-mismatch")
        assert v.domain == "monster"
    ...
    def test_retag_fires_for_deeper_field_defenses(self, access):
        ...
        v = next(x for x in check(_sheet([_monster(stat_block=sb)]), access)
                 if x.code == "companion-defense-subset-violation")
        assert v.domain == "monster"
    ...
    def test_retag_uses_correct_index_for_second_monster(self, access):
        ...
        sb = _clean_stat_block(index=1)
        sb["speed"] = {"walk": 99}
        sheet = _sheet([_monster(index=0), _monster(stat_block=sb, index=1)])
        v = next(x for x in check(sheet, access)
                 if x.code == "companion-speed-mismatch")
        assert v.domain == "monster"
...
# ── state compatibility (reused, re-tagged) ──────────────────────────────────
...
class TestStateCompatibility:
    def test_incompatible_states_fire(self, access):
        ...
        assert any(x.code == "companion-state-incompatible" and x.domain == "monster" for x in v)
    ...
    def test_compatible_states_do_not_fire(self, access):
        ...
        v = check(_sheet([_monster(stat_block=sb)]), access)
        assert not any(x.code == "companion-state-incompatible" for x in v)
