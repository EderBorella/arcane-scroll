...
- **5-schema contract split + partial Phase B + T30 DB fixes (F05-T29/T30).** **Phase A**: single v10 sheet split into 5 independently versioned schemas (PR #70). **CORE validator**: 12-domain check via path adapter on `POST /validate-core`. **Phase B partial**: condition_kind on 8 grant tables, item_slot dim (13 slots), state_compatibility table (14 rows + access layer). **T30 DB fixes**: `build_l18_fixes.py` script (B3/B8/B10/B11) — condition_effect (46 rows, 15 conditions), cumulative_max_seconds on magic_item (2 items), Mystic Arcanum grants (4 rows), pact_slot migration (17 rows). Suite: 474 passed + 8 xfail.
