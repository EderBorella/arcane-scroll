...
- **Deriver/validator residuals (F05-T113 partial / T114 / T117 / T118).** **T114** — the CORE deriver + resources check now gather feat- and subclass-owned `grant_resource` (9 DB rows previously dropped; subclass grants gate on the *class* level, not total level, so a high-level subclass grant can't leak into a low-subclass-level multiclass build); 40 gold CORE sheets gained legitimate budget entries. **T117** — `hit_points.max_reduction` and `hit_points.form_temp_pool` added to the non-overwritable set so a live variable-drain/transform value survives a gap-fill re-derive. **T118** — the deriver import-guard test's `banned_names` corrected to the actual rule-resolver names (`_resolve_speeds`/`_resolve_senses`); the two previously-listed gather helpers no longer exist post-T96. **T113 (partial)** — Action Surge and Indomitable modelled as `class_resource` count ladders using the existing generic tooling (book-verified: Action Surge 1→2 at L17; Indomitable 1@L9→2@L13→3@L17), exposing and fixing a real errata (a level-16 Fighter with Action Surge max 2 → 1). **985 tests, 0 xfailed**; gold-v23 harness green. **Deferred/surfaced:** **T122** (orphan-budget flagging) proved premature — a corpus audit showed ~136 budget entries across ~a dozen class/species features the DB has never modelled (Arcane Recovery, Bardic Inspiration, Lay on Hands, Mystic Arcanum, Innate Sorcery, …), plus key-quality debt (casing/format fragmentation; a `Combat Superiority`→`Superiority Dice` errata). Orphan flagging can only land after those are modelled — a resource-completion epic (oddity by-level pools via deriver/validator functions + DB existence markers; Mystic Arcanum → GRIMOIRE; species/subclass trait grants; key normalisation) is next, with T122 as its capstone.
